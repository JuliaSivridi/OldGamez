...
async def menu_new_duel(callback: CallbackQuery, user, lang) -> None:
    ...
    invite_message = await callback.message.answer(
        ...
        reply_markup=duel_invite_keyboard(lang, session.join_code or "", game_name=lang["game-bj"]),
    )
...
async def callback_group_join(callback: CallbackQuery) -> None:
    ...
    players.append({"id": user.id, "name": get_display_name(user)})
