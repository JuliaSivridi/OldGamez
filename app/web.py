...
async def _load_data() -> dict:
    async with SessionLocal() as sess:
        ...
        game_summary = [
            {"game": row.game_code, "played": int(row.played or 0), "wins": int(row.wins or 0)}
            for row in r.all()
        ]
...
def _render_html(data: dict, generated_at: str) -> str:
    ...
    summary_table = table(
        ["Game", "Played", "Wins"],
        [[s["game"], s["played"], s["wins"]] for s in summary],
    )

    users_table = table(
        ["ID", "Name", "Username", "Lang", "Current game", "Last seen"],
        [
            [u["id"], u["name"], u["username"], u["lang"], u["current_game"], _fmt(u["updated"])]
            for u in users
        ],
    )
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h2 { color: #a0d8b3; margin: 28px 0 10px; font-size: 15px; border-bottom: 1px solid #333; padding-bottom: 4px; }
